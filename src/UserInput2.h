...
// namespace StarterPack { namespace UserInput2 {
...
namespace UserInput2 {
    ...
    UserInputDeviceBase *KeyHandler = nullptr;

    void assignHandler( UserInputDeviceBase &handler ) {
        KeyHandler = &handler;
    }
    ...
    inline static bool isKeyPressed( char key, bool issueWaitForKeyUp = true ) {
    ...
    inline static uint8_t getKeyDown( char *allowedKeys = nullptr ) {
        if ( KeyHandler == nullptr ) return 0;
        return KeyHandler->getKeyDown( allowedKeys );
    }

    inline static uint8_t getContinuousKey( char *allowedKeys = nullptr ) {
        if ( KeyHandler == nullptr ) return 0;
    ...
    // inline static uint8_t getRepeatingKey( char *allowedKeys = nullptr, bool okayKeyAlsoRepeating = false ) {
    //     if ( KeyHandler == nullptr ) return 0;
    //     return KeyHandler->getRepeatingKey( allowedKeys, okayKeyAlsoRepeating );
    // }

    // inline static uint8_t getRepeatingKey( bool okayKeyAlsoRepeating, char *allowedKeys = nullptr ) {
    //     if ( KeyHandler == nullptr ) return 0;
    //     return KeyHandler->getRepeatingKey( allowedKeys, okayKeyAlsoRepeating );
    // }
